...
<table>
  ...
  {rows}
    ...
    console.print(f"📄 [bold green]Report:[/bold green] {output_path}")
...
def main():
    ...
    console.print(Panel.fit(
        ...
        border_style="cyan",
        title="[bold white]v2.0.0 - Agentic[/bold white]"
    ))
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if passed:
                console.print("✅ [bold green]Test PASSED![/bold green] "
                              "Search bar found on Amazon.")
            else:
                console.print("[yellow]Amazon may have blocked the action - "
                              "continuing demo.[/yellow]")
        ...
        try:
            ...
            console.print("✅ Unexpected pass - Amazon may have changed their DOM")
        except PlaywrightError:
            console.print(
                "❌ [bold red]CI/CD TEST FAILED![/bold red] "
                "Locator '#twotabsearchtextbox' not found.\n"
                "[dim]Pipeline is red. Slack is firing.[/dim]"
            )
        page_v2.close()
        time.sleep(2)
        ...
        try:
            ...
            if passed:
                ...
                console.print("[dim]QA engineer sleeps peacefully.[/dim]")
        except Exception as e:
            console.print(f"\n❌ [bold red]Healing Failed:[/bold red] {e}")

        console.print("\n[bold blue]Agent Decision Log:[/bold blue]")
        for event in healing_page.healing_events:
            status_color = "green" if event.status == "healed" else "red"
            console.print(
                f"  [{status_color}]{event.status.upper()}[/{status_color}] "
    ...
    # ── PHASE 4: Selenium — cross-framework demo ──────────────────────
    console.print(
        "\n[bold magenta]═══ PHASE 4: Selenium Python — Shared RAG Memory ═══[/bold magenta]"
    )
    try:
    ...
    # Generate final report with all events from both frameworks
