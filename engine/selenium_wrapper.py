"""
engine/selenium_wrapper.py

Self-Healing Selenium WebDriver wrapper for Cognitive Healium.
Uses only the real HealiumMemory API: query_vector_memory + save_to_vector_memory.
Runtime cache is handled by the providers.py CacheProvider (same as Playwright SDK).
"""
...
from engine.models import HealingEvent

from langchain_groq import ChatGroq
from langchain_core.messages import HumanMessage

# SSL bypass - same patch as healing_agent.py
import httpx
...
RESPOND ONLY WITH VALID JSON. No markdown. No explanation outside the JSON:
{
  "suggestions": [
    ...
    {
      "locator_type": "XPATH",
      "locator_value": "//input[@placeholder='Search']",
      "confidence": 0.87,
      "reasoning": "Placeholder text is stable across UI changes"
    }
  ]
}
...
class SelfHealingDriver:
    """
    Wraps a Selenium WebDriver with self-healing locator capabilities.

    Shares the same ChromaDB memory (RAG) and runtime cache as the Playwright SDK -
    so a healing learned in Playwright is immediately available to Selenium tests.
    ...
    Usage:
        ...
        memory = HealiumMemory(tenant_id="my-team")

        driver = webdriver.Chrome()
        h = SelfHealingDriver(driver, memory=memory, cache_provider=cache)
        h.fill(By.ID, "broken-id", "hello", intent="search bar input")
        h.click(By.CSS_SELECTOR, ".broken-btn", intent="submit search button")
    """
    ...
    def fill(self, by: By, value: str, text: str, intent: str = ""):
        ...
        import time
        time.sleep(0.5)  # Small wait for element to be ready
        try:
          element.clear()
        except Exception:
          pass  # Element might not need clearing
