...
from mcp_client import MCPBrowserClient, _resolve_to_playwright_locator
...
# ==========================================================
# SSL BYPASS: Force httpx (used by Groq/LangChain) to skip 
# certificate verification. Required for corporate networks.
# ==========================================================
import httpx
...
def build_healing_agent(memory, page, cache, storage, tenant_id: str):
    ...
    def llm_reason(state: HealingState) -> dict:
        ...
        try:
            ...
            return {
                ...
                "agent_log": [
                    f"LLM: {len(suggestions)} suggestions, "
                    f"top confidence {top:.0%}"
                ],
            }
    ...
    def verify_playwright(state: HealingState) -> dict:
        ...
        try:
            ...
            if storage:
                try:
                    screenshot_url = storage.save_screenshot(
                        tenant_id, locator, idx
                    )
    ...
    def route_after_cache(
        state: HealingState,
    ) -> Literal["__end__", "capture_context"]:
    ...
    g.add_conditional_edges("verify_playwright", route_after_verify, {
        END:                END,
        "verify_playwright": "verify_playwright",
        "escalate":          "escalate",
    })
...
class SelfHealingPage:
    ...
    def _run(self, action: str, selector: str, intent: str,
        ...
        if healed:
            ...
            logger.info(
                f"HEALED: {selector} -> {final_state['healed_locator']}"
            )
            if action == "inner_text":
                try:
                    locator = _resolve_to_playwright_locator(self._page, final_state["healed_locator"])
